...
public class MyCamera extends Camera implements IRegisterIOTCListener {
	...
	public boolean isRequestReconnected = false;
	...
	public IOTCAPIs mIOTCAPIs;
	public int Check_Device_Result = -1;


	public MyCamera(Context context , String name, String uid, String acc, String pwd) {
		super(context);
		mName = name;
		if(uid!=null)
		{
	...
	public String toString() {
		return "MyCamera{" +
				...
				", mPwd='" + mPwd + '\'' +
				", isMapping=" + isMapping +
				", isRequestReconnected=" + isRequestReconnected +
				", mEventCount=" + mEventCount +
				", nIsSupportTimeZone=" + nIsSupportTimeZone +
				", nGMTDiff=" + nGMTDiff +
				", szTimeZoneString=" + Arrays.toString(szTimeZoneString) +
				", bIsMotionDetected=" + bIsMotionDetected +
				", bIsIOAlarm=" + bIsIOAlarm +
				", mUUID=" + mUUID +
				", mStreamDefs=" + mStreamDefs +
				", mIOTCAPIs=" + mIOTCAPIs +
				", Check_Device_Result=" + Check_Device_Result +
				'}';
	}
	...
	public void receiveIOCtrlData(final Camera camera, final int avChannel, int avIOCtrlMsgType, final byte[] data) {
		...
		if (avIOCtrlMsgType == AVIOCTRLDEFs.IOTYPE_USER_IPCAM_GETSUPPORTSTREAM_RESP) {
			...
			System.arraycopy(data, 12, szTimeZoneString, 0, data.length-12);
			
				
			try {
				/*Log.i("szTimeZoneString", new String(szTimeZoneString,0, szTimeZoneString.length,"unicode"));*/
