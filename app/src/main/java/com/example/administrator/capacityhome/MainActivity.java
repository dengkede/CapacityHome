...
public class MainActivity extends BaseActivity implements View.OnClickListener, BaiduMap.OnMapStatusChangeListener {
    ...
    // 要申请的权限
    private String[] permissions = {Manifest.permission.DISABLE_KEYGUARD, Manifest.permission.WAKE_LOCK};
                            ...
                            if (jsonObject.getString("order_id") != null) {
                                if (is_put) {
                                    ...
                                    if (MyApplication.can_detail) {
                                        ...
                                        startActivity_Bundle(MainActivity.this, OrderDetailActivity.class, bundle);
                                    }
                                } else {
                                    //取件
                                    if (MyApplication.can_detail) {
...
//        drawerLayout.closeDrawer();
    ...
    private void setBrand() {
        if (MyApplication.getBrand() == 0) {
            ...
            if (MyApplication.getBrand_pics() != null) {
                Log.d("brand_pic", MyApplication.getBrand_pics());
                Picasso.with(this).load(RequestTag.BaseImageUrl + MyApplication.getBrand_pics()).error(R.mipmap.default_iv).placeholder(R.mipmap.default_iv).into(img_brand);
            } else {
                //根据品牌id获取图片
                getBrand();
            }
    ...
    private void setStyle() {
        ...
        map_view.setCustomMapStylePath(MyApplication.getAppContext().getFilesDir().getAbsolutePath() + "/" + "map_style.txt");
    }
                ...
                if (getAgrement_laundry == null) {
                    agreement_onclick = new Agreement_Dialog.Onclick() {
                        ...
                        public void OnClickLisener(View v) {
                            ...
                            startActivity_Bundle(MainActivity.this, MoneyDetailActivity.class, bundle);
                        }
                    };
...
//                getAgrement_price.getAggrement_Detail(null, "PricingRules", "我知道了");
    ...
    private void setUserDate() {
        try {
            ...
            Picasso.with(this).load(pic).placeholder(R.mipmap.default_iv).error(R.mipmap.default_iv).transform(new CircleTransform()).into(home_imgUser, new Callback() {
                ...
                public void onError() {
                    try {
                        Picasso.with(MainActivity.this).load(RequestTag.BaseImageUrl + MyApplication.getConfigrationJson().getJSONObject("other").getJSONArray("head").get(MyApplication.getUserJson().getInt("sex"))).placeholder(R.mipmap.default_iv).error(R.mipmap.default_iv).transform(new CircleTransform()).into(home_imgUser);
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
                }
            ...
            home_tvBalance.setText(MyApplication.getUserJson().getDouble("money") / 1000 + "");
            home_tvClothes.setText(MyApplication.getUserJson().getDouble("clothes") / 1000 + "");
            ...
            public void onSuccess(int statusCode, JSONObject response) {
                try {
                    ...
                    if (response.getInt("status") != 0) {
                        ...
                        if (MyApplication.getBrand_pics() != null) {
                            Picasso.with(MainActivity.this).load(RequestTag.BaseImageUrl + MyApplication.getBrand_pics()).error(R.mipmap.default_iv).placeholder(R.mipmap.default_iv).into(img_brand);
                        }
                    }
                } catch (Exception e) {
                    toastMessage("数据解析异常，请稍后重试");
    ...
    public void onBackPressed() {
        if (number == 0) {
            Toast.makeText(MainActivity.this, "再次点击退出小e管家", Toast.LENGTH_SHORT).show();
        ...
        switch (requestCode) {
            ...
            case Constant.REQUEST_CODE_ASK_CALL_PHONE:
                if (grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                    ...
                    Toast.makeText(MainActivity.this, "请至权限中心打开本应用的电话拨打访问权限", Toast.LENGTH_LONG).show();
                }
                break;
            ...
            listener = new OnGetRoutePlanResultListener() {
                ...
                public void onGetWalkingRouteResult(WalkingRouteResult result) {
                    ...
                    if (result == null || result.error != SearchResult.ERRORNO.NO_ERROR) {
                        Toast.makeText(MainActivity.this, "抱歉，太远了不支持步行导航", Toast.LENGTH_SHORT).show();
                        isGuihua = false;
                        return;
                    }
...
//                });
            ...
            public void initFailed() {
                Toast.makeText(MainActivity.this, "请检查网络设置和GPS是否打开", Toast.LENGTH_SHORT).show();
            }
...
//                            line2.setVisibility(View.VISIBLE);
    ...
    private void addOrderView(final JSONObject jsonObject) {
        ...
        view.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                v.startAnimation(new AnimationUtils().loadAnimation(MainActivity.this, R.anim.bg_alpha));
                Bundle bundle = new Bundle();
                try {
            ...
            public void onSuccess(int statusCode, JSONObject response) {
                try {
                    ...
                    if (response.getInt("status") != 0) {
                        ...
                        if (MyApplication.getBrand_pics() != null) {
                            Picasso.with(MainActivity.this).load(RequestTag.BaseImageUrl + MyApplication.brand_pics).error(R.mipmap.default_iv).placeholder(R.mipmap.default_iv).into(img_brand);
                        }
                        //   Log.d("urlll",pic);
                    }
                } catch (Exception e) {
