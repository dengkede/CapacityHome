...
//    // device that
    ...
    public static final int AVIOTC_WIFIAPMODE_NULL = 0x00;
    public static final int AVIOTC_WIFIAPMODE_MANAGED = 0x01;
    public static final int AVIOTC_WIFIAPMODE_ADHOC = 0x02;
    ...
    /* AVIOCTRL Remove files */
    public static final int IOTYPE_USER_IPCAM_REMOVE_FILE_REQ = 0x614;
    public static final int IOTYPE_USER_IPCAM_REMOVE_FILE_RESP = 0x615;
    ...
    /* AVIOCTRL WDR settings */
    public static final int IOTYPE_USER_IPCAM_GET_WDR_REQ = 0x60C;
    public static final int IOTYPE_USER_IPCAM_GET_WDR_RESP = 0x60D;
    public static final int IOTYPE_USER_IPCAM_SET_WDR_REQ = 0x60E;
    public static final int IOTYPE_USER_IPCAM_SET_WDR_RESP = 0x60F;

    /* AVIOCTRL Time lapse recording */
    public static final int IOTYPE_USER_IPCAM_GET_TIMELAPSE_REQ = 0x610;
    public static final int IOTYPE_USER_IPCAM_GET_TIMELAPSE_RESP = 0x611;
    public static final int IOTYPE_USER_IPCAM_SET_TIMELAPSE_REQ = 0x612;
    public static final int IOTYPE_USER_IPCAM_SET_TIMELAPSE_RESP = 0x613;

    /* AVIOCTRL OTA FW upgrade */
    ...
    /* Device sleep IOCtrl */
    public static final int IOTYPE_USER_IPCAM_DEVICESLEEP_REQ = 0x720;
    public static final int IOTYPE_USER_IPCAM_DEVICESLEEP_RESP = 0x721;
    ...
    /* AVIOCTRL Device Liveview Timesatemp */
    public static final int IOTYPE_USER_IPCAM_SET_LIVEVIEWTIMESTAMP_REQ = 0X802;
    public static final int IOTYPE_USER_IPCAM_SET_LIVEVIEWTIMESTAMP_RESP = 0X803;
    public static final int IOTYPE_USER_IPCAM_GET_LIVEVIEWTIMESTAMP_REQ = 0X804;
    public static final int IOTYPE_USER_IPCAM_GET_LIVEVIEWTIMESTAMP_RESP = 0X805;

    /* AVIOCTRL Device Record Duration */
    public static final int IOTYPE_USER_IPCAM_GET_CLOUDRECORD_REQ = 0X0732;
    public static final int IOTYPE_USER_IPCAM_GET_CLOUDRECORD_RESP = 0X0733;
    public static final int IOTYPE_USER_IPCAM_SET_CLOUDRECORD_REQ = 0X0734;
    public static final int IOTYPE_USER_IPCAM_SET_CLOUDRECORD_RESP = 0X0735;


    /* AVIOCTRL Device Wifi strength */
    public static final int IOTYPE_USER_IPCAM_GET_WiFiSIGNAL_REQ = 0X0730;
    public static final int IOTYPE_USER_IPCAM_GET_WiFiSIGNAL_RESP = 0X0731;

    /* Request device to clean buf and decrease video rate when cache is too big*/
    public static final int IOTYPE_USER_IPCAM_SET_CLEANBUFFER_RATIO_REQ  = 0xFF010501;
    public static final int IOTYPE_USER_IPCAM_SET_CLEANBUFFER_RATIO_RESP = 0xFF010502;
    ...
    public static class SMsgAVIoctrlUploadResp {
        ...
        public SMsgAVIoctrlUploadResp(byte[] data) {
            channel = Packet.byteArrayToShort_Little(data, 0);
            result = Packet.byteArrayToShort_Little(data, 4);
            purpose = data[8];
        }
    }
    ...
    public static class SMsgAVIoctrlPtzCmd {
        byte control;   // refer to PTZ Command
        byte speed;     // ptz control speed
        byte point;     // no use in APP so far. preset position, for RS485 PT
        byte limit;     // no use in APP so far.
        byte aux;       // no use in APP so far. auxiliary switch, for RS485 PT
    ...
    public static class SMsgAVIoctrlPlayRecord {
        ...
        public static byte[] parseContent (int channel, int command, int param, long time) {
            ...
            byte[] timedata = STimeDay.parseContent(cal.get(Calendar.YEAR), cal.get(Calendar.MONTH), cal.get(Calendar.DAY_OF_MONTH), cal.get
                    (Calendar.DAY_OF_WEEK), cal.get(Calendar.HOUR_OF_DAY), cal.get(Calendar.MINUTE), cal.get(Calendar.SECOND));
    ...
    public class SMsgAVIoctrlEvent {
        ...
        int channel;     // Camera Index
        int event;       // refer to Event Type
        byte[] reserved = new byte[4];
    }

    public static class SMsgAVIoctrlSetVideoModeReq {
        int channel; // Camera Index
        byte mode;   // refer to VIDEO MODE
        byte[] reserved = new byte[3];
...
//			byte[] channel_byte = channel_str.getBytes();
    ...
    public static class SMsgAVIoctrlGetFlowInfoReq {
        public int channel;            // camera index
        public int collect_interval;   // seconds of interval to collect flow information
    ...
    public static class SMsgAVIoctrlGetFlowInfoResp {
        public int channel;            // camera index
        public int collect_interval;   // seconds of interval client will collect flow information
    ...
    public static class SMsgAVIoctrlCurrentFlowInfo {
        public int channel;                        // camera index
        public int total_frame_count;              // Total frame count in the specified interval
        public int lost_incomplete_frame_count;    // Total lost and incomplete frame count in the specified interval
        public int total_expected_frame_size;      // Total expected frame size from avRecvFrameData2()
        public int total_actual_frame_size;        // Total actual frame size from avRecvFrameData2()
        public int elapse_time_ms;                 // Timestamp in millisecond of this report.

        public static byte[] parseContent (int channel, int total_frame_count, int lost_incomplete_frame_count, int total_expected_frame_size, int
                total_actual_frame_size, int elapse_time_ms) {
    ...
    public static class SMsgAVIoctrlTimeZone {
        ...
        public int nIsSupportTimeZone;                    // 0 = flase, 1 = true;
        public int nGMTDiff;                              // the difference between GMT in minutes
    ...
    public static class SMsgAVIoctrlGetDropboxResp { // for resp
        public short nSupportDropbox;  // 0:no support/ 1: support dropbox
        public short nLinked;          // 0:no link/ 1:linked
        public String szLinkUDID;      // Link UDID for App

        public SMsgAVIoctrlGetDropboxResp (byte[] data) {
...
//			szLinkUDID = new String(data, 4, data.length-4).replace("\0","");
    ...
    public static class SMsgAVIoctrlSetDropbox {
        ...
        byte[] szAccessToken = new byte[128];         // Oauth token
        byte[] szAccessTokenSecret = new byte[128];   // Oauth token secret
        byte[] szAppKey = new byte[128];              // App Key (reserved)
        byte[] szSecret = new byte[128];              // Secret  (reserved)
        byte[] reserved = new byte[4];
...
//			index += reserve.length;
    ...
    public static class SMsgAVIoctrlSetPresetReq {
        int channel;     // Camera Index
        int nPresetIdx;  // index to storage
    ...
    public static class SMsgAVIoctrlCruiseStart {
        int channel; // Camera Index
        byte mode;   /* Cruise mode :
                      *    1. AVIOCTRL_CRUISEMODE_VERTICAL = 0x00
                      *    2. AVIOCTRL_CRUISEMODE_HORIZONTAL = 0x01
                      */
        byte[] reserved = new byte[3];
    ...
    public static class SMgAVIoctrlGetBrightResp {
        int channel; // Camera Index
        byte bright; // refer to Brightness Settings
        byte[] reserved = new byte[3];
    ...
    public static class SMsgAVIoctrlSetBrightReq {
        int channel;     // Camera Index
        byte bright;     /*  Brightness Settings:
                          *  1. LEVEL_LOWEST  = 5;
                          *  2. LEVEL_LOW     = 4;
                          *  3. LEVEL_MEDIUM  = 3;
                          *  4. LEVEL_HIGH    = 2;
    ...
    public static class SMgAVIoctrlGetContrastResp {
        int channel;    // Camera Index
        byte contrast;  // refer to Contrast Settings
        byte[] reserved = new byte[3];
    }

    public static class SMsgAVIoctrSetContrastReq {
        int channel;     // Camera Index
        byte contrast;   /*  Contrast Settings:
                          *  1. LEVEL_LOWEST  = 5;
                          *  2. LEVEL_LOW     = 4;
                          *  3. LEVEL_MEDIUM  = 3;
                          *  4. LEVEL_HIGH    = 2;
    ...
    public static class SMsgAVIoctrlSetFtpReq {
        int channel;                      // Camera Index
        byte[] ftpServer = new byte[68];  // FTP Server Name
        int ftpPort;                      // Port
        byte[] userName = new byte[20];   // Account Name
        byte[] password = new byte[20];   // Camera Password
        byte[] path = new byte[68];       // FTP Url Path
        int passiveMode;                  // 0 - off, 1 - on

        // total 188 bytes
        public static byte[] parseContent (int nchannel, String ftpServer, int nftpPort, String userName, String password, String path, int
                passiveMode) {
            ...
            System.arraycopy(ch, 0, result, 0, 4);
            System.arraycopy(ftpServer.getBytes(), 0, result, 4, ftpServer.getBytes().length);  // input length : 68
            System.arraycopy(port, 0, result, 72, 4);
            System.arraycopy(userName.getBytes(), 0, result, 76, userName.getBytes().length);   // input length : 20
            System.arraycopy(password.getBytes(), 0, result, 96, password.getBytes().length);   // input length : 20
            System.arraycopy(path.getBytes(), 0, result, 116, path.getBytes().length);          // input length : 68
    ...
    public static class SMsgAVIoctrlGetFtpResp {
        int channel;                     // Camera Index
        byte[] ftpServer = new byte[68]; // ex:10.1.1.1
        int ftpPort;                     // ex:21
        byte[] userName = new byte[20];  // Account Name
        byte[] password = new byte[20];  // Password
        byte[] path = new byte[68];      // FTP Url Path
        int passiveMode;                 // 0 - off, 1 - on
        // total 188 bytes
    ...
    public static class SMsgAVIoctrlExGetSmtpResp {
        ...
        byte[] user = new byte[32];       // account
        byte[] pwd = new byte[32];        // password
    }
    ...
    public static class SMsgAVIoctrlExSetSmtpReq {
        ...
        public static byte[] parseContent (int channel, String sender, String receiver1, String server, int port, int mail_tls, String user, String
                pwd) {
    ...
    public static class SAvFile {
        ...
        // 0x02: No Recording file in the event
        byte type;      // type of file 0:video, 1:photo
        byte length;     // path's length
        byte[] FilePath = new byte[72];  // file's path
        ...
        public SAvFile(byte[] data) {
            byte[] start = new byte[8];
            System.arraycopy(data, 0, start, 0, 8);
            stTime = new STimeDay(start);
            event = data[8];
            status = data[9];
            type = data[10];
            length = data[11];
            System.arraycopy(data, 12, FilePath, 0, 72);
     ...
     */
    public static class SMsgAVIoctrlFileListResp {
        int channel;      // Camera Index
        int total;        // Total event amount in this search session
        byte index;        // package index, 0,1,2...;
        ...
        byte count;        // how much events in this package
        byte type;       // type of file 0:video, 1:photo
        SAvFile[] stFile;      // The first memory address of the events in this package
        ...
        public SAvFile[] getStFile() {
            return stFile;
        }

        public void setStFile(SAvFile[] stFile) {
            this.stFile = stFile;
        }

        public SMsgAVIoctrlFileListResp(byte[] data) {
            channel = Packet.byteArrayToShort_Little(data, 0);
            total = Packet.byteArrayToShort_Little(data, 4);
            index = data[8];
            endflag = data[9];
            count = data[10];
            type = data[11];
            ...
            if (count > 0) {
                stFile = new SAvFile[count];
                int i = 0;

                byte[] tmp = new byte[count * 84];
                System.arraycopy(data, 12, tmp, 0, count * 84);

                byte[] buf = new byte[84];

                for (i = 0; i < count; ++i) {
                    System.arraycopy(tmp, i * 84, buf, 0, 84);
                    stFile[i] = new SAvFile(buf);
                }
            }
    ...
    public static class SMsgAVIoctrlSetClientRecvReadyResp {
        ...
        int channel;      // Camera Index
        int result;       // 0: success ; != 0: failure
    }
    ...
    public static class SMsgAVIoctrlUploadReq {
        int channel; // AvServer Index
        int length;  // path length
        byte[] path = new byte[72]; // file path
        byte purpose;  // 0: for download file, 1: for show photo(NOVATEK_DOWNLOAD_FILE or NOVATEK_DOWNLOAD_PHOTO)
        byte[] reserved = new byte[7];

        public static byte[] parseContent(int channel, int length, byte[] path, byte purpose) {
            byte[] data = new byte[88];

            byte[] ch = Packet.intToByteArray_Little(channel);
            System.arraycopy(ch, 0, data, 0, 4);

            byte[] len = Packet.intToByteArray_Little(length);
            System.arraycopy(len, 0, data, 4, 4);

            System.arraycopy(path, 0, data, 8, 72);
    ...
    /**
     * IOTYPE_USER_IPCAM_REMOVE_FILE_REQ = 0x614;
     */
    public static class SMsgAVIoctrlRemoveFileReq {
        ...
        public static byte[] parseContent (int channel, String path) {
            byte[] result = new byte[80];

            byte[] ch = Packet.intToByteArray_Little(channel);
            System.arraycopy(ch, 0, result, 0, 4);
            byte[] length = Packet.intToByteArray_Little(path.length());
            System.arraycopy(length, 0, result, 4, 4);
            System.arraycopy(path.getBytes(), 0, result, 8, path.length());
    ...
    /**
     * IOTYPE_USER_IPCAM_REMOVE_FILE_RESP = 0x615;
     */
    public static class SMsgAVIoctrlRemoveFileResp {
        int channel;    // Camera Index
        byte result;    // 0 : failed ; 1 : succeed
        byte[] reserved = new byte[3];
    }
    ...
    public static class SMsgAVIoctrlGetBatteryResp {
        int channel;    // Camera Index
        byte battery;   // 0 – 100 (%) ; -1 : charging
        byte[] reserved = new byte[3];
    }

    /**
     * IOTYPE_USER_IPCAM_GET_WDR_REQ = 0x60C;
     */
    ...
    /**
     * IOTYPE_USER_IPCAM_GET_WDR_RESP = 0x60D;
     */
    public static class SMsgAVIoctrlGetWDRResp {
        int channel;    // Camera Index
        byte enable;    // 0 : off ; 1 : open
        byte[] reserved = new byte[3];
    }

    /**
     * IOTYPE_USER_IPCAM_SET_WDR_REQ = 0x60E;
     */
    public static class SMsgAVIoctrlSetWDRReq {
        ...
        public static byte[] parseContent (int channel, byte enable) {
            byte[] result = new byte[8];
    ...
    /**
     * IOTYPE_USER_IPCAM_SET_WDR_RESP = 0x60F;
     */
    ...
    /**
     * IOTYPE_USER_IPCAM_GET_TIMELAPSE_REQ = 0x610;
     */
    ...
    /**
     * IOTYPE_USER_IPCAM_GET_TIMELAPSE_RESP = 0x611;
     */
    public static class SMsgAVIoctrlGetTimeLapseResp {
        int channel;       // Camera Index
        byte timeLapse;    // refer to Time Lapse Interval
        byte[] reserved = new byte[3];
    }

    /**
     * IOTYPE_USER_IPCAM_SET_TIMELAPSE_REQ = 0x612;
     */
    public static class SMsgAVIoctrlSetTimeLapseReq {
        ...
        public static byte[] parseContent (int channel, byte interval) {
            byte[] result = new byte[8];
    ...
    /**
     * IOTYPE_USER_IPCAM_SET_TIMELAPSE_RESP = 0x613;
     */
    ...
    /**
     * IOTYPE_USER_IPCAM_STARTFWUPGRADE_REQ = 0x700;
     */
    ...
    /**
     * IOTYPE_USER_IPCAM_STARTFWUPGRADE_RESP = 0x701;
     */
    ...
    /**
     * IOTYPE_USER_IPCAM_GET_CLOUDRECORD_REQ = 0X732;
     */
    ...
    /**
     * IOTYPE_USER_IPCAM_GET_CLOUDRECORD_RESP = 0X733;
     */
    public static class SMsgAVIoctrlGetCloudRecordResp {
        int recordmode; //0 10s //1 20s // 2 30s
        byte[] reserved = new byte[4];
    }

    /**
     * IOTYPE_USER_IPCAM_SET_CLOUDRECORD_REQ = 0X734;
     */

    public static class SMsgAVIoctrlSetCloudRecordReq {
        int channel; // Camera Index
        byte recordmode;// 0 10s // 1 20s //2 30s
        byte[] reserved = new byte[3];

        public static byte[] parseContent (int channel,byte recordmode) {
    ...
    /**
     * IOTYPE_USER_IPCAM_SET_CLOUDRECORD_RESP = 0X735;
     */
    ...
    public static class SMsgAVIoctrlGetWifiSignalResp {
        int channel; // Camera Index
        int result; //bad -71~ -100 //normal -41~-70  //good 0~-40

        byte[] reserved = new byte[3];
    }


    public static class SMsgAVIoctrlSetCleanBufferRatioReq {

        int status;
        int ratio;   // set Clean Buffer Ratio and Response Resend Buffer Ratio
        int bufSize; // in KB

        public static byte[] parseContent(int ratio) {
            byte[] result = new byte[12];
            byte[] cmdId = Packet.intToByteArray_Little(ratio);
            System.arraycopy(cmdId, 0, result, 4, 4);
    ...
    public static class SMsgAVIoctrlSetCleanBufferRatioResp {
